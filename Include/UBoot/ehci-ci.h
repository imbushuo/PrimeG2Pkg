 ...
 */
struct usb_ehci {
	UINT32	id;		/* 0x000 - Identification register */
	UINT32	hwgeneral;	/* 0x004 - General hardware parameters */
	UINT32	hwhost;		/* 0x008 - Host hardware parameters */
	UINT32	hwdevice;	/* 0x00C - Device hardware parameters  */
	UINT32	hwtxbuf;	/* 0x010 - TX buffer hardware parameters */
	UINT32	hwrxbuf;	/* 0x014 - RX buffer hardware parameters */
	UINT8	res1[0x68];
	UINT32	gptimer0_ld;	/* 0x080 - General Purpose Timer 0 load value */
	UINT32	gptimer0_ctrl;	/* 0x084 - General Purpose Timer 0 control */
	UINT32     gptimer1_ld;	/* 0x088 - General Purpose Timer 1 load value */
	UINT32     gptimer1_ctrl;	/* 0x08C - General Purpose Timer 1 control */
	UINT32	sbuscfg;	/* 0x090 - System Bus Interface Control */
	UINT32	sbusstatus;	/* 0x094 - System Bus Interface Status */
	UINT32	sbusmode;	/* 0x098 - System Bus Interface Mode */
	UINT32	genconfig;	/* 0x09C - USB Core Configuration */
	UINT32	genconfig2;	/* 0x0A0 - USB Core Configuration 2 */
	UINT8	res2[0x5c];
	UINT8	caplength;	/* 0x100 - Capability Register Length */
	UINT8	res3[0x1];
	UINT16	hciversion;	/* 0x102 - Host Interface Version */
	UINT32	hcsparams;	/* 0x104 - Host Structural Parameters */
	UINT32	hccparams;	/* 0x108 - Host Capability Parameters */
	UINT8	res4[0x14];
	UINT32	dciversion;	/* 0x120 - Device Interface Version */
	UINT32	dciparams;	/* 0x124 - Device Controller Params */
	UINT8	res5[0x18];
	UINT32	usbcmd;		/* 0x140 - USB Command */
	UINT32	usbsts;		/* 0x144 - USB Status */
	UINT32	usbintr;	/* 0x148 - USB Interrupt Enable */
	UINT32	frindex;	/* 0x14C - USB Frame Index */
	UINT8	res6[0x4];
	UINT32	perlistbase;	/* 0x154 - Periodic List Base
					 - USB Device Address */
	UINT32	ep_list_addr;	/* 0x158 - Next Asynchronous List
					 - End Point Address */
	UINT8	res7[0x4];
	UINT32	burstsize;	/* 0x160 - Programmable Burst Size */
#define FSL_EHCI_TXPBURST(X)	((X) << 8)
#define FSL_EHCI_RXPBURST(X)	(X)
	UINT32	txfilltuning;	/* 0x164 - Host TT Transmit
					   pre-buffer packet tuning */
	UINT8	res8[0x8];
	UINT32	ulpi_viewpoint;	/* 0x170 - ULPI Reister Access */
	UINT8	res9[0xc];
	UINT32	config_flag;	/* 0x180 - Configured Flag Register */
	UINT32	portsc;		/* 0x184 - Port status/control */
	UINT8	res10[0x1C];
	UINT32	otgsc;		/* 0x1a4 - Oo-The-Go status and control */
	UINT32	usbmode;	/* 0x1a8 - USB Device Mode */
	UINT32	epsetupstat;	/* 0x1ac - End Point Setup Status */
	UINT32	epprime;	/* 0x1b0 - End Point Init Status */
	UINT32	epflush;	/* 0x1b4 - End Point De-initlialize */
	UINT32	epstatus;	/* 0x1b8 - End Point Status */
	UINT32	epcomplete;	/* 0x1bc - End Point Complete */
	UINT32	epctrl0;	/* 0x1c0 - End Point Control 0 */
	UINT32	epctrl1;	/* 0x1c4 - End Point Control 1 */
	UINT32	epctrl2;	/* 0x1c8 - End Point Control 2 */
	UINT32	epctrl3;	/* 0x1cc - End Point Control 3 */
	UINT32	epctrl4;	/* 0x1d0 - End Point Control 4 */
	UINT32	epctrl5;	/* 0x1d4 - End Point Control 5 */
	UINT8	res11[0x28];
	UINT32	usbgenctrl;	/* 0x200 - USB General Control */
	UINT32	isiphyctrl;	/* 0x204 - On-Chip PHY Control */
	UINT8	res12[0x1F8];
	UINT32	snoop1;		/* 0x400 - Snoop 1 */
	UINT32	snoop2;		/* 0x404 - Snoop 2 */
	UINT32	age_cnt_limit;	/* 0x408 - Age Count Threshold */
	UINT32	prictrl;	/* 0x40c - Priority Control */
	UINT32	sictrl;		/* 0x410 - System Interface Control */
	UINT8	res13[0xEC];
	UINT32	control;	/* 0x500 - Control */
	UINT8	res14[0xafc];
};
