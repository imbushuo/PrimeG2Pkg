...
UINTN gWidth = FixedPcdGet32(PcdFrameBufferWidth);
UINTN gHeight = FixedPcdGet32(PcdFrameBufferHeight);
UINTN gBpp    = 32;
char* gFrameBuffer = NULL;
...
SerialPortInitialize(VOID)
{
  ...
  gFrameBuffer = (char *) (UINTN) MmioRead32((UINTN) &pLcdIfRegs->hw_lcdif_cur_buf_reg);
...
void FbConScrollUp(void)
{
  unsigned short *dst   = (void*) gFrameBuffer;
